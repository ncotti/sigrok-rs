...
impl SrOption {
    pub fn new(p_option: *const sr_option) -> SrOption {
        ...
        let name: String = unsafe{CStr::from_ptr(option.name)}.to_string_lossy().to_string();
        let id: String = unsafe{CStr::from_ptr(option.id)}.to_string_lossy().to_string();
        let description: String = unsafe{CStr::from_ptr(option.desc)}.to_string_lossy().to_string();
...
impl OutputModule {
    ...
    fn new(p_module: *const sr_output_module) -> OutputModule {
        ...
        let name: String = unsafe{ CStr::from_ptr(sr::sr_output_name_get(p_module))}.to_string_lossy().to_string();
        let description: String = unsafe{ CStr::from_ptr(sr::sr_output_description_get(p_module))}.to_string_lossy().to_string();
        let id: String =  unsafe{ CStr::from_ptr(sr::sr_output_id_get(p_module))}.to_string_lossy().to_string();
